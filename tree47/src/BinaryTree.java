...
public class BinaryTree {
    ...
    private Node recursiveDeleteCopy(Node current){
        ...
        if ((current.left != null) && (current.right != null)) {
            copy.left = recursiveDeleteCopy(current.left);
            copy.right = recursiveDeleteCopy(current.right);
        }
        if ((current.left == null) && (current.right != null)){
            copy.left = recursiveDeleteCopy(new Node(-1));
            copy.right = recursiveDeleteCopy(current.right);
        }
        if ((current.left != null) && (current.right == null)){
            copy.left = recursiveDeleteCopy(current.left);
            copy.right = recursiveDeleteCopy(new Node(-1));
        }
        return copy;
    }
